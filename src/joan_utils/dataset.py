...
from joan_utils.utils import remove_extra_spaces, split_verdicts_comments_amit
import os
...
class SocialNormDataset:
    ...
    def __init__(self, sc, sn, cond=5):
        self.verdictToId = {}
        self.idToVerdict = []
        self.idToParent = [] #dataset.verdictToAuthor[dataset.idToVerdict[id]]
        self.verdictToText = {}
        self.verdictToParent = {}
        self.verdictToCleanedText = {}
        self.verdictToLabel = {}
        ...
        self.number_of_posts = len(set(self.idTopostId))
        self.number_of_annotations = len(list(self.verdictToLabel.keys()))
...
class RegularDataset:
    """Creates the verdicts datastructures to use through out the project:
        For a dataframe of the shape: rows = texts, columns = (text, author_ids_[0,1,...,N]) and each entry is a n authors verdict
    """
    def __init__(self, df):
        ...
        self.verdictToAuthor = {}
        self.postToVerdicts = ListDict()
        # self.aita_labels = [0,1] #['NTA','YTA']
        self.authors = [col_idx for col_idx in df.columns if re.fullmatch(r'[0-9]+', col_idx)]
    ...
    def load_maps(self, df):
        for _, row in tqdm(df.iterrows(), desc='Creating situations maps'):
            post_id = row['id']
            text = row['text']
            ...
            for author in self.authors:
                ...
                if author_label == author_label and author_label >= 0: # not float('nan')
                    verdict_id = f"{post_id}_{author}"
                    #1
                    self.postIdToText[post_id] = text
                    self.idTopostId.append(post_id)
                    self.idToParent.append(post_id)
                    self.postIdToId[post_id] = len(self.idTopostId) - 1
                    #2
                    self.verdictToAuthor[verdict_id] = author
                    self.authorsToVerdicts.append(author, verdict_id)
                    #3
                    self.idToVerdict.append(verdict_id)
                    self.verdictToId[verdict_id] = len(self.idToVerdict) - 1
                    self.verdictToText[verdict_id] = text
                    self.verdictToLabel[verdict_id] = author_label
                    self.verdictToParent[verdict_id] = post_id
  
                     
        self.verdictToCleanedText = self.verdictToText
        self.verdictToTokensLength = {k: len(v.split(' ')) for k, v in self.verdictToCleanedText.items()}
