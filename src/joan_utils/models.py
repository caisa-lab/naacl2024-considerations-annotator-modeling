...
class SentBertClassifier_Anchor_v2(nn.Module):
    def __init__(self, num_outputs=2, sbert_dim=384, 
        ...
        self.linear1 = nn.Linear(sbert_dim, sbert_dim//2)
        self.linear2 = nn.Linear(sbert_dim//2, num_outputs)
        self.combine_linear = nn.Linear((sbert_dim//2)*2, num_outputs)
            
        self.relu = nn.ReLU()
        
        
    def forward(self, input, anchor_input=None, is_anchor_tensor=None, only_anchor=False):
        bert_output = self.model(**input)
        pooled_output = self.mean_pooling(bert_output, input['attention_mask'])
        downsized_output = self.linear1(self.dropout(pooled_output))
        downsized_output_relu = self.relu(downsized_output)
        
        if only_anchor:
            return downsized_output_relu
        
        output = self.linear2(self.dropout(downsized_output_relu))
        
        if anchor_input != None:
            non_anchor_tensor = downsized_output_relu[~is_anchor_tensor,:]
            anchor_downsized_bert_for_non_anchors = anchor_input[~is_anchor_tensor,:] # anchor_logits.masked_select(~is_anchor_tensor)
            
            combined = torch.concat([non_anchor_tensor, anchor_downsized_bert_for_non_anchors], dim=1)
            combined_output = self.combine_linear(combined)
            output[~is_anchor_tensor,:] = combined_output
...
class SentBertClassifier_Anchor_v4(nn.Module):
    def __init__(self, num_outputs=2, sbert_dim=384, 
        ...
        self.linear1 = nn.Linear(sbert_dim, sbert_dim//2)
        self.linear2 = nn.Linear(sbert_dim//2, num_outputs)
        self.combine_linear = nn.Linear(4, 2)
            
        self.relu = nn.ReLU()
        
        
    def forward(self, input, anchor_input=None, is_anchor_tensor=None):
        bert_output = self.model(**input)
        pooled_output = self.mean_pooling(bert_output, input['attention_mask'])
        downsized_output = self.linear1(self.dropout(pooled_output))
        output = self.relu(downsized_output)
        ...
        if anchor_input != None:
            ...
            anchor_logits_for_non_anchors = anchor_input[~is_anchor_tensor,:] # anchor_logits.masked_select(~is_anchor_tensor)
